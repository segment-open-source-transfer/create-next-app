...
const cloneCustomRepo = require('./utils/clone-repo')
...
module.exports = function createNextApp(opts) {
  ...
  // User can specify an example OR a clone-repo option (not both)
  if (opts.cloneRepo && opts.example) {
    console.log(messages.exampleOrClone())
    process.exit(1)
  }
  ...
  } else if (opts.cloneRepo) {
    cloneCustomRepo({
      projectName: projectName,
      repoPath: opts.cloneRepo
    }).then(installWithMessageFactory(opts))
  } else {
